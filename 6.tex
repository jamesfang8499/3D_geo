...
\begin{enumerate}
    ...
    \item 
用一个平面截直面三角的三个面，得一截面是三角形，求证这三角形的垂心，是这三面角的顶点在这平面内的射影.
    \item 
...
$\therefore\quad \Sigma <n\cdot 180^{\circ}-2n90^{\circ}+360^{\circ}=360^{\circ}$.
...
这里说的连续变形，可理解为橡皮变形，设想图形在橡皮膜上，当橡皮膜变形时，有的地方伸长，有的地方压缩，但不破裂，不折叠. 这样橡皮膜上的图形也跟着改变，这种图形的变化过程就是连续变形. 例如正六面体可连续变形成为一个球（图3.8），所以正六面体是一简单体. 同理，棱柱、棱锥、棱台、圆锥等凸多面体都是简单多面体.
...
多面体中，也有不是简单多面体的. 例如长方体穿透个洞所得的多面体，连续变形后就不能变为一个球面而只能变成一个环面（图3.9）.
\begin{figure}[htp]
    \centering
...
如果我们用$V$表示简单多面的顶点数，$E$表示这多面体的棱数，$F$表示这多面体的面数. 请同学们数一下下面这些简单多面体的$V$、$E$、$F$各是多少？它们之间有什么关系吗？
...
\[
\angle VEO=\arcsin\frac{VO}{VE}=\arcsin\frac{\frac{\sqrt{2}}{2}a}{\frac{\sqrt{3}}{2}a}=\frac{\sqrt{6}}{3}\]
\[
